...
public class MyBatisTest {
    ...
    @Test
    public void testScopeSelect() throws IOException {
        SqlSession session = sqlSessionFactory.openSession(); // ExecutorType.BATCH
        try {
            ScopeMapper mapper = session.getMapper(ScopeMapper.class);
            BusScope busScope = mapper.selectById(1);
            System.out.println(busScope.getBusinessScope());
        } finally {
            session.close();
        }
    }
    @Test
    public void testScopeInsert() throws IOException {
        SqlSession session = sqlSessionFactory.openSession(); // ExecutorType.BATCH
        try {
            ScopeMapper mapper = session.getMapper(ScopeMapper.class);
            list.add(1);
            list.add(2);
            list.add(4);
            mapper.insertScope(new BusScope(1,list));
            session.commit();
        } finally {
            session.close();
        }
